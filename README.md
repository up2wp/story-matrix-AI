...
2. 在同一页面选择图片存储方式：默认本地存储；如启用 Immich，需填写 Immich 服务地址、API Key 和项目名称。项目名称会自动映射为 Immich 相册，系统会创建或复用同名相册。
3. 在 **系统设置 → 用户级功能权限** 中为普通用户授权“作品生图”。拥有者和管理员在全局开关开启后默认可用。
4. 作者打开作品后进入 **视觉资产**，选择角色、章节和提示词类型，生成草稿或手写提示词。
5. 检查并保存提示词后选择已启用模型生成图片；失败时提示词和模型选择会保留，可复制或重试。

V1 支持角色高清面部特写、章节服饰、章节道具、多视角全身图四类新建提示词；历史作品中的“章节服饰/道具”旧类型会继续展示和参与上下文引用。章节相关提示词只使用标题、摘要、场景信息和小段摘录，不会把整章正文发送给 AI。

生成图片默认落到服务端受控资产目录；启用 Immich 后，系统会在调用生图 Provider 前先检查 Immich 服务可达、API Key 有效且具备上传能力，检查失败不会消耗生图额度。Immich 上传成功后，作品只保存 asset id、文件名、存储状态和同源后端代理地址；浏览器不会拿到 Immich 服务地址或 API Key，也不会直连 Immich。生成成功但 Immich 上传失败时，服务端会自动重试，失败后保留可恢复状态供用户重试。作品 JSON 不保存上游临时 URL、API Key 或整段 base64。当前版本不做额度统计、人脸一致性评分、自动挑图、批量重绘，也不会自动写入角色头像或章节插图。
