...
export default function ChapterAudiobookPanel({ work, chapter, writing }: Props) {
  ...
  const [hasDirtySegments, setHasDirtySegments] = useState(false)
  const {
    audiobook,
    segmentChapter,
    updateSegment,
  ...
  const playSegmentAudio = async (segment: typeof segments[number]) => {
    if (!segment.generationId) return
  ...
  const downloadSegmentAudio = async (segment: typeof segments[number]) => {
    if (!segment.generationId) return
    const url = await voiceboxClient.fetchMediaUrl(voiceboxClient.audioUrl(segment.generationId))
    const anchor = document.createElement('a')
    anchor.href = url
    anchor.download = `${chapter.title}-${segment.order + 1}-${segment.speakerName}.wav`.replace(/[\\/:*?"<>|]/g, '_')
    ...
    >
      <Space direction="vertical" size="middle" style={{ width: '100%' }}>
        <Space wrap>
          ...
          {unresolvedCount > 0 && <Button disabled={writing || hasDirtySegments || segmentingChapterId === chapter.id || generatingChapterId === chapter.id} onClick={() => approveReviewSegments(chapter.id)}>确认复核无误</Button>}
          <Button type="primary" icon={<SoundOutlined />} loading={generatingChapterId === chapter.id} disabled={writing || hasDirtySegments || segmentingChapterId === chapter.id || !segments.length || missing.length > 0 || segments.some((segment) => segment.attributionStatus === 'failed')} onClick={() => generateChapterAudio(chapter)}>生成章节音频</Button>
        ...
        <ChapterAudioPlayer chapter={chapter} segments={segments} chapterAudio={audiobook.chapterAudio[chapter.id]} onSynthesized={(jobId) => saveSynthesizedChapterAudio(chapter.id, jobId)} />
      </Space>
    </Card>
  )
}
