...
import type { AudiobookSegment, Chapter, ChapterAudioState } from '@/core/types'
...
interface Props {
  ...
  chapterAudio?: ChapterAudioState
  onSynthesized?: (jobId: string) => Promise<void>
}

export default function ChapterAudioPlayer({ chapter, segments, chapterAudio, onSynthesized }: Props) {
  ...
  return (
    <Card size="small" title="章节音频" extra={<Button icon={<DownloadOutlined />} loading={synthesizing} onClick={synthesizeChapterAudio}>合成章节音频</Button>}>
      <Space direction="vertical" style={{ width: '100%' }}>
        ...
        {chapterAudio?.status === 'completed' && chapterAudio.chapterAudioJobId && <Text type="success">章节音频已合成，可在全文预览标题处播放。</Text>}
