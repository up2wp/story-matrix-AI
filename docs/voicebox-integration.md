...
1. 选择已有正文的章节。
2. 点击 **AI 分段**。系统会先用规则切出可编辑初稿，再按小窗口批次归因低置信度对白。
3. 查看分段进度；如果出现低置信度、未知 speaker 或归因失败，先在分段表中修正 speaker，或对失败片段点击 **重试归因**。
4. 检查并编辑分段表中的 speaker、文本、语气 / Prompt。
5. 点击 **保存修改**，确认未保存提示消失。
6. 点击 **生成章节音频**。生成章节音频、失败片段重试、一键生成语气和合并分段都只基于已保存分段运行；一键生成语气默认不会覆盖已手写的 Prompt。
7. 生成完成后在线播放或下载章节清单；如果某些片段生成失败，可只重试失败片段。
...
AI 分段现在不是整章一次性黑盒请求：规则切段会先保存草稿，低置信度片段再分批调用 AI 归因。若某批次失败，已成功片段会保留在分段表中。

处理方式：

- 检查管理后台 AI 服务配置是否可用。
- 对红色“归因失败”片段点击 **重试归因**。
- 如果 AI 仍无法判断，直接在分段表手动选择 speaker；手动修正后即可继续生成章节音频。
- 低置信度提示不代表原文丢失，只表示建议人工复核。
- 如果正文是“声音/动作/表情描写 + 引号对白”的句式，规则切段应先把引号内对白拆成待复核片段；若整句仍被归为旁白，说明规则切段需要补样例。
