...
- 接口开发时默认优先传最小内容，禁止单次请求发送/接收整章内容；视觉资产的章节候选抽取和草稿生成是明确例外，可由服务端读取完整章节并发送给系统配置的文本 AI Provider，但浏览器请求、API 响应、视觉资产缓存、提示词记录、图片记录和日志仍不得携带完整章节正文或长摘录
- 注重用户体验，且需要考虑移动端便捷性
- 提交信息用中文但仍然携带 feat、fix 等前缀
- 额外接入的能力要考虑到扩展性，例如语音可能不止接入 voicebox，生图模型也不止接入 OpenAI、MiniMax
- 开始前读取 ROADMAP.md，开发任务结束后更新 ROADMAP.md，但不需要记录日期、不要无脑堆加条目，相似条目进行合并
